...
ALPHA = 0.9 # Change filter coefficient based on experiments please!!!
...
def process_buffers_and_send(raw_x, raw_y, raw_z):
    ...
    try:
        ...
        print(f'Uploaded to ThingsBoard | peak_amp={peakmag_z:.4f} peak_amp over thershold-{amp_detected}')        
        print(f'Uploaded to ThingsBoard | peak_freq={peakfreq_z:.4f} peak_freq in detection range-{freq_detected}')        
        return True
    except OSError as e:
